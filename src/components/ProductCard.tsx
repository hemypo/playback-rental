...
const ProductCard = ({
  ...
  return <Link to={`/product/${product.id}`} state={{
    ...
    scrollTop: true
      ...
      <Card className={`overflow-hidden transition-all hover:shadow-md ${featured ? 'border-primary/20' : ''}`}>
        <div className="relative aspect-square overflow-hidden bg-gray-100">
          <ProductImage imageUrl={product.imageUrl} title={product.title} className="h-full w-full object-cover object-center transition-transform duration-300 group-hover:scale-105" />
          {featured && <div className="absolute top-2 left-2 bg-primary text-white text-xs font-medium px-2 py-1 rounded">
              Популярное
            </div>}
          {product.available === false && <div className="absolute inset-0 bg-black/60 flex items-center justify-center">
        ...
        <CardFooter className="p-4 pt-0 flex items-center justify-between">
          ...
          <Button size="sm" variant={hasBookingDates ? "default" : "outline"} className="rounded-full" onClick={handleAddToCart} disabled={product.available === false}>
